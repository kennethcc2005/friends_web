...
export default class OutsideTripList extends React.Component {
  ...
  render() {
    ...
    const rightIconMenu =(poiId) => {
        return (
          <IconMenu iconButtonElement={iconButtonElement}>
            <MenuItem onClick={() => this.props.onSuggestEvent(poiId)}>Resuggest</MenuItem>
            <MenuItem onClick={() => this.props.onDeleteEvent(poiId)}>Delete</MenuItem>
          </IconMenu>
        )
    };
    ...
    for(var i=0; i<this.props.outsideRouteDetails.length; i++) {
      ...
      fullDetails.push(
        <ListItem
          ...
          rightIconButton={rightIconMenu(keyId)}
